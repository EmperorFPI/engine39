    ...
    {elseif $_post['post_type'] == "funding" && $_post['funding']}
      <div class="mt10">
        ...
        <div class="post-funding-meta">
          ...
          {*Add in the chaity raising money for here*}
			      <div class="text mt5">Raising donations for: {$_post['funding']['charity']}</div>
    ...
    {if $_post['post_type'] == "product" && $_post['product']}
      <div class="post-product-container">
        ...
        {if $_post['product']['available'] && $_post['author_id'] != $user->_data['user_id'] }
          <div class="mt10 d-grid">
            <button type="button" class="btn btn-info js_chat-start" data-uid="{$_post['author_id']}" data-name="{$_post['post_author_name']}" data-link="{$_post['user_name']}" data-picture="{$_post['post_author_picture']}">
              {__("Contact Seller")}
            </button>
          </div>
        {/if}
