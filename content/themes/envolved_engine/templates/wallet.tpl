...
<div class="page-header">
  ...
  <div class="inner">
    <h2>{__("Wallet")}</h2>
    <p class="text-xlg">{__("Your Personal Virtual Foundation")}</p>
    <p class="text-lg">{__("Contributions are Tax Deductible")}</p>
    <p class="text-lg">{__("Give any amount to any Charity or Fundraiser at Anytime!")}</p>
                    ...
                    <table class="table table-striped table-bordered table-hover js_dataTable">
                      ...
                      <tbody>
                        {foreach $transactions as $transaction}
                          <tr>
                            <td><div class="table-responsive-sm">{$transaction['transaction_id']}</div></td>
                            <td><div class="table-responsive-sm">
                            {if $transaction['type'] == "out"}
  {if $transaction['node_type'] == "donation"}
    <span class="badge rounded-pill badge-lg bg-info mr5 donation"><i class="far fa-arrow-alt-circle-right"></i></span>
  {else}
...
{if $transaction['type'] == "out"}
                            ...
                            <td><div class="table-responsive-sm">
                              {if $transaction['type'] == "out"}
                                {if $transaction['node_type'] == "donation"}
                                  <span class="badge rounded-pill badge-lg bg-info mr10">{__("Donated to")}</span>
                            ...
                            <td><div class="table-responsive-sm"><span class="js_moment" data-time="{$transaction['date']}">{$transaction['date']}</span></div></td>
                          </tr>
                        {/foreach}
                      </tbody>
                    </table>
