  ...
  {else}
    ...
    {if $page == "index" || $page == "profile" || $page == "page" || $page == "group" || $page == "event" || $page == "post" || $page == "photo" || $page == "market" || $page == "blogs" || $page == "directory" || $page == "search" || $page ==
      ...
      {/if}
            ...
            {if $system['tips_enabled']}
        <!-- Donations -->
        <script id="send-donation" type="text/template">
          <div class="modal-header">
          <h6 class="modal-title">
  {include file='__svg_icons.tpl' icon="tips" class="main-icon mr10" width="24px" height="24px"}
  {__("Send Donation to")} {$spage['page_title']}
</h6>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <form class="js_ajax-forms" data-url="payments/wallet.php?do=send_donation">
            <div class="modal-body">
            <div class="form-group">
                <label class="form-label" for="amount">{__("Available Balance")}</label>
                <div>
                  <span class="badge badge-lg bg-info">{print_money($user->_data['user_wallet_balance']|number_format:2)}</span>
                </div>
              </div>
            ...
            <div class="modal-footer">
              ...
              <input type="hidden" name="user_id" value="{literal}{{id}}{/literal}">
              <button type="button" class="btn btn-light" data-bs-dismiss="modal">{__("Cancel")}</button>
              <button type="submit" class="btn btn-primary">{__("Send")}</button>
            </div>
          </form>
