...
class User
{
      ...
      while ($notification = $get_notifications->fetch_assoc()) {
        ...
        switch ($notification['action']) {
          ...
          case 'tip_sent':
            ...
            $notification['url'] = $system['system_url'] . '/wallet';
            $notification['message'] = __("Sent a TIP ") . " " . print_money($notification['node_type']);
            break;
  ...
  public function post_notification($args = [])
  {
    ...
    if ($system['onesignal_notification_enabled'] && $receiver['onesignal_user_id']) {
      ...
      switch ($action) {
        ...
        case 'tip_sent':
          $notification['url'] = $system['system_url'] . '/wallet';
          $notification['message'] = __("Sent a TIP - ") . " " . print_money($node_type);
          break;
  ...
  public function publisher($args = [])
  {
    ...
    switch ($post['post_type']) {
      ...
      case 'funding':
        ...
        /* Note: no need to return any data as publisher will redirect to post link */
        $db->query(sprintf("INSERT INTO posts_funding (post_id, title, charity, amount, cover_image) VALUES (%s, %s, %s, %s, %s)", secure($post['post_id'], 'int'), secure($args['funding']->title), secure($args['funding'] ->charity), secure($args['funding']->amount), secure($args['funding']->cover_image))) or _error('SQL_ERROR_THROWEN');
        break;
  ...
  public function check_page_adminship($user_id, $page_id)
  {
    ...
    /**
   * check_page_donations
   * 
   * @param integer $page_id
   * @return boolean
   */
  public function get_donations($page_id)
  {
    global $db;
    /* if ($this->_logged_in) { */
      $get_donations = $db->query(sprintf("SELECT SUM(amount) AS amount FROM donations WHERE page_id = %s", secure($page_id, 'int'))) or _error('SQL_ERROR_THROWEN');
      if ($get_donations->num_rows > 0) {
        return $get_donations->fetch_assoc()['amount'];
        /* var_dump($donation_amount); */ // Debug statement
...
{
  global $db;
  $get_user_donations = $db->query(sprintf("SELECT SUM(amount) AS amount FROM donations WHERE user_id = %s", secure($user_id, 'int'))) or _error('SQL_ERROR_THROWEN');
  if ($get_user_donations->num_rows > 0) {
    $donation_amount = $get_user_donations->fetch_assoc()['amount'];
...
public function wallet_send_donation($page_id, $amount)
{
  global $db, $system;
  /* check if wallet enabled */
  if (!$system['wallet_enabled']) {
  ...
  /* validate amount */
  if (is_empty($amount) || !is_numeric($amount) || $amount <= 0) {
    throw new Exception(__("You must enter a valid amount of money"));
  }
  if ($amount < $system['tips_min_amount']) {
    throw new Exception(__("The amount is less than the minimum amount allowed"));
  }
  if ($amount > $system['tips_max_amount']) {
    throw new Exception(__("The amount is more than the maximum amount allowed"));
  }
  /* validate target page 
  if (is_empty($page_id) || !is_numeric($page_id)) {
    throw new Exception(__("Invalid page"));
  } */
 /* $check_page = $db->query(sprintf("SELECT COUNT(*) as count FROM pages WHERE page_id = %s", secure($page_id, 'int'))) or _error('SQL_ERROR_THROWEN');
  if ($check_page->fetch_assoc()['count'] == 0) {
    throw new Exception(__("Invalid page"));
  ...
  // $db->query("INSERT INTO `donations` (`user_id`, `page_id`, `amount`) VALUES ('$this->_data['user_id']', '$page_id', '$amount')");
  $stmt = $db->prepare("INSERT INTO `donations` (`user_id`, `page_id`, `amount`) VALUES (?, ?, ?)");
  $user_id = $this->_data['user_id'];
  $stmt->bind_param("iii", $user_id, $page_id, $amount);
  $stmt->execute();
  
  /* log this transaction */
  $this->wallet_set_transaction($this->_data['user_id'], 'donation', $page_id, $amount, 'out');

  /* wallet transaction */
  // $this->wallet_set_transaction($page_id, 'donation', $this->_data['user_id'], $amount, 'in');
}
...
/*   /**
  ...
  /** My wallet_get_transactions code to add pages 62323 */
/**
 * wallet_get_transactions
 * 
 ...
 * @return array
 */
public function wallet_get_transactions($nodeType = null, $nodeId = null, $amount = null)
{
    global $db;
    $transactions = [];
    
    // Construct the SQL query
    $sql = "SELECT wallet_transactions.*, users.user_name, users.user_firstname, users.user_lastname, users.user_gender, users.user_picture, pages.page_title
            FROM wallet_transactions 
            LEFT JOIN users ON (wallet_transactions.node_type='user' OR wallet_transactions.node_type='tip') AND wallet_transactions.node_id = users.user_id
            LEFT JOIN pages ON wallet_transactions.node_type='donation' AND wallet_transactions.node_id = pages.page_id
            WHERE wallet_transactions.user_id = %s";
    
    // Add additional conditions for donation if provided
    if ($nodeType == 'donation' && $nodeId && $amount) {
        $sql .= " AND wallet_transactions.node_type = 'donation' AND wallet_transactions.node_id = %s AND wallet_transactions.amount = %s";
    }
    
    //$sql .= " ORDER BY wallet_transactions.transaction_id DESC";
    
    // Debug statement to display the SQL query
    // echo "SQL Query: " . $sql . "<br>";
    
    // Execute the query with the provided parameters
    $get_transactions = $db->query(sprintf($sql, secure($this->_data['user_id'], 'int'), secure($nodeId, 'int'), secure($amount, 'float'))) or _error('SQL_ERROR_THROWEN');
  ...
  public function settings($edit, $args)
  {
    ...
    switch ($edit) {
      ...
      case 'privacy':
        ...
        if (!in_array($args['user_privacy_poke'], $privacy) || !in_array($args['user_privacy_gifts'], $privacy) || !in_array($args['user_privacy_wall'], $privacy) || !in_array($args['user_privacy_donations'], $privacy) || !in_array($args['user_privacy_gender'], $privacy) || !in_array($args['user_privacy_relationship'], $privacy) || !in_array($args['user_privacy_birthdate'], $privacy) || !in_array($args['user_privacy_basic'], $privacy) || !in_array($args['user_privacy_work'], $privacy) || !in_array($args['user_privacy_location'], $privacy) || !in_array($args['user_privacy_education'], $privacy) || !in_array($args['user_privacy_other'], $privacy) || !in_array($args['user_privacy_friends'], $privacy) || !in_array($args['user_privacy_followers'], $privacy) || !in_array($args['user_privacy_photos'], $privacy) || !in_array($args['user_privacy_pages'], $privacy) || !in_array($args['user_privacy_groups'], $privacy) || !in_array($args['user_privacy_events'], $privacy)) {
          _error(400);
        }
        /* update user */
        $db->query(sprintf(
          ...
          secure($args['user_privacy_donations']),
