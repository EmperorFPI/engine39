 ...
 */    if (arguments[0] == "#modal-login") {
    ...
    } else if(arguments[0] == "#chat-calling" || arguments[0] == "#chat-ringing") {
...
$(function () {
  ...
  /* Wallet Donation Added by FPI*/
  $('body').on('click', '.js_payment-wallet-donation', function () {
    var _this = $(this);
    /* button loading */
    button_status(_this, "loading");
    ...
    /* post the request */
    $.post(api['payments/wallet'], { 'do': 'wallet_donation_payment', 'page_id': _this.data('id') }, function (response) {
      /* button reset */
      button_status(_this, "reset");
      /* check the response */
      if (!response) return;
      ...
      .fail(function () {
        ...
        button_status(_this, "reset");
        /* handle error */
        modal('#modal-message', { title: __['Error'], message: __['Donation Failed'] });
      });
  });
