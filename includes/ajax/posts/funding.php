...
try {
  ...
  switch ($_REQUEST['do']) {
    ...
    case 'publish':
      ...
      /* check charity name -- this will change to search our database for attached charities */
      if (is_empty($_POST['charity'])) {
        return_json(array('error' => true, 'message' => __("Add charity for your fundraiser")));
      }
      if (strlen($_POST['charity']) < 3) {
        return_json(array('error' => true, 'message' => __("Minimum charity title is 3 characters")));
      }
      if (strlen($_POST['charity']) > 100) {
        return_json(array('error' => true, 'message' => __("Maximum charity title is 100 characters")));
      }
